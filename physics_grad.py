...
class inverted_pendulum_physics(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        ...
        new_state = np.array([pa.data.qpos[0], pa.data.qpos[1], pa.data.qpos[2], pa.data.qvel[0], pa.data.qvel[1], pa.data.qvel[2]])
        new_state_tensor = torch.tensor(new_state, requires_grad=True, dtype=torch.float32).view(1, 6)
