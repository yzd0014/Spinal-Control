...
class PendulumEnv(gym.Env):
    ...
    def __init__(self, control_type = Control_Type.BASELINE):
        ...
        # for i in range(0.4, 0.4, 0.05):
        #     self.target_candidates.append(i)
        ...
        #target angle, parent site x, child site z, child site x offset
    ...
    def step(self, action):
        ...
        pos_error_penalty = -np.linalg.norm(0 - self.data.qpos[0])
        mag_penalty = -np.linalg.norm(np.array([self.data.ctrl[1], self.data.ctrl[2]]))
        reward = pos_error_penalty

        self.ticks += 1
        if self.ticks >= self.episode_length:
            self.done = True

        observation = np.array([0, 0.1, 0.6, 0.06])
        info = {}
    ...
    def my_baseline(self, model, data):
        action = np.array([self.m_ctrl[0], self.m_ctrl[1]])
    ...
    def my_neuron_controller(self, model, data):
        action = np.array([self.m_ctrl[0], self.m_ctrl[1], self.m_ctrl[2], self.m_ctrl[3]])
